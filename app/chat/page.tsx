...
export default function ChatPage() {
  ...
  // 'sidebar' = chat list, 'chat' = message wall
          ...
          const unsubscribeUser = docSnapshot(
            ...
            (userSnap) => {
              ...
              setChatRooms((prev) => {
                ...
                if (exists) return prev.map((r) => r.id === roomId ? updated : r)
                return [...prev, updated]
              })
            }
          )
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // For regular users, go straight to chat view on mobile
      setMobileView('chat')
  ...
  const handleSelectRoom = (room: ChatRoom) => {
    ...
    // Switch to message wall on mobile when a room is selected
    setMobileView('chat')
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Message wall — always visible on desktop, conditionally on mobile */}
        <div className={`
